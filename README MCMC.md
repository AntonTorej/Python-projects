...
$$p(h_{1:T}, \mu, \phi, \sigma \mid r_{1:T}) \propto p(r_{1:T} \mid h_{1:T}) p(h_{1:T} \mid \mu, \phi, \sigma) p(\mu, \phi, \sigma)$$
...
$$\ell_{\text{local}}(h_t) = -\tfrac{h_t}{2} - \frac{r_t^2}{2e^{h_t}} - \frac{(h_t - \mu - \phi(h_{t-1}-\mu))^2}{2\sigma^2} - \frac{(h_{t+1} - \mu - \phi(h_t-\mu))^2}{2\sigma^2}$$

and the move is accepted with probability $\alpha = \min(1, \exp[\ell_{\text{local}}(h_t') - \ell_{\text{local}}(h_t)])$.
...
Fit to daily market excess returns (Mkt-RF, Fama–French, 2004–2024). The
...
|-----------|----------|-------------------------|
| $\mu$     | ≈ −9.1  | −9 to −10 |
| $\phi$    | ≈ 0.965 | 0.95–0.98 (high persistence) |
| $\sigma$  | ≈ 0.16  | 0.1–0.3 |
...
$r_{T+k} \sim \mathcal{N}(0, e^{h_{T+k}})$ — a forecast of the **range** (risk),
not the direction. The model makes no claim to predict whether the market rises
or falls, by construction, since $\mathbb{E}[r_t] = 0$.
