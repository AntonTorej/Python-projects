...
variance of the return is $\operatorname{Var}(r_t \mid h_t) = e^{h_t}$.
...
\underbrace{p(r_{1:T} \mid h_{1:T})}_{\text{observation}}\;
\underbrace{p(h_{1:T} \mid \mu, \phi, \sigma)}_{\text{AR(1) dynamics}}\;
\underbrace{p(\mu, \phi, \sigma)}_{\text{prior}}
$$

has no closed form, so it is sampled with MCMC. Each sweep alternates two blocks:
...
the unnormalised log-posterior is ever evaluated. The proposal for $h_t$ uses the
local log-density
...
Fit to daily market excess returns ($\text{Mkt-RF}$, Fama–French, 2004–2024). The
...
Daily Fama–French research factors ($\text{Mkt-RF}$ column) from the
